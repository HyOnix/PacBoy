...
typedef struct {
    ...
    int hauteurImage;
    unsigned char
        *donneesRGB; /* Donnees sauvees dans l'ordre bleu-vert-rouge */
} DonneesImageRGB;
...
/* Fonction essayant de lire le fichier passe en parametre, et renvoyant une
   structure
        contenant les informations de l'image en cas de succes, NULL sinon */
DonneesImageRGB *lisBMPRGB(char *nom);

/* Fonction ecrivant les informations de l'image dans le fichier passe en
   parametre. Renvoie faux en cas de probleme, vrai sinon */
